...
優先度①サイトの掲載情報に価値があり、将来的にpv数（kpi）につながるか？
優先度②factチェック（いまはココだけ）
それとgptは門番というより、アドバイザーの立ち位置の方が、Akiraのやりたようにできるのではないか？
（私が最初にgpt役割をのfactチェックで指示してしまったので、factチェックしなきゃ、が強すぎて本来の目的を見失ってないか？）
claudeエンジニア＝コーダー
gpt＝アドバイザー（ビジネス視点でAkiraに意見する）
gemini＝アドバイザー（利用者視点でAkiraに意見する）
意見は課題として、当日中に処理する必要はない。課題はクリティカル（まずいもの）をのぞき、翌日以降に持ち越し。
当日の作成コンテンツは前日の課題もふまえてAkiraが自由に決められる方がいいかな、と考えた。
